...
<section class="border-b border-slate-200 pb-10 md:pb-14">
  ...
  <?php else: ?>
    <div class="mt-5 grid gap-5 lg:grid-cols-[minmax(0,1.8fr)_minmax(280px,1fr)]">
      <div class="featured-swiper swiper relative w-full min-w-0 max-w-full overflow-hidden rounded-xl"><div class="swiper-wrapper"><?php foreach ($featured as $index => $item): ?><div class="swiper-slide group relative overflow-hidden rounded-xl bg-slate-950 text-white"><article><img class="absolute inset-0 h-full w-full object-cover" src="<?= htmlspecialchars($item['hero_image']) ?>" alt="" width="1200" height="675" <?= $index === 0 ? 'fetchpriority="high"' : 'loading="lazy"' ?>><div class="absolute inset-0 bg-linear-to-t from-slate-950/94 via-slate-950/22 to-transparent" aria-hidden="true"></div><div class="relative flex min-h-80 flex-col justify-end p-7 md:min-h-112 md:p-10"><p class="text-xs font-bold tracking-widest text-orange-200 uppercase"><?= htmlspecialchars($item['category_name']) ?></p><h1 class="mt-3 max-w-3xl break-words text-3xl leading-tight font-extrabold tracking-tight text-balance md:text-5xl"><a href="/<?= $item['category'] ?>/<?= $item['slug'] ?>" class="group-hover:text-orange-100"><?= htmlspecialchars($item['title']) ?></a></h1><p class="mt-4 max-w-2xl text-slate-200"><?= htmlspecialchars($item['excerpt']) ?></p><p class="mt-5 text-sm text-slate-300"><?= htmlspecialchars($item['published_at']) ?> · <?= $index + 1 ?>/<?= count($featured) ?></p></div></article></div><?php endforeach; ?></div><button class="featured-prev absolute top-1/2 left-3 z-10 grid size-10 -translate-y-1/2 place-items-center rounded-full bg-white/90 text-slate-900 shadow-md backdrop-blur transition hover:bg-white md:left-4" aria-label="Article précédent">←</button><button class="featured-next absolute top-1/2 right-3 z-10 grid size-10 -translate-y-1/2 place-items-center rounded-full bg-white/90 text-slate-900 shadow-md backdrop-blur transition hover:bg-white md:right-4" aria-label="Article suivant">→</button><div class="featured-pagination mt-4 text-center"></div></div>
...
<?php foreach ($categorySpotlights as $index => $spotlight): $main = $spotlight['main']; ?>
  <section class="border-b border-slate-200 py-10 md:py-14">
    ...
    <div class="mt-5 grid gap-8 lg:grid-cols-[minmax(0,1.3fr)_minmax(280px,1fr)]">
      <article>
        <img class="h-52 w-full rounded-xl object-cover" src="<?= htmlspecialchars($main['hero_image'] ?? '') ?>" alt="" width="960" height="540" loading="lazy">
