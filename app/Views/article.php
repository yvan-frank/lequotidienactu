...
<article class="min-w-0">
  ...
  <p class="mt-5 flex flex-wrap items-center gap-2 text-xs font-bold tracking-widest text-brand-600 uppercase">
    ...
    <?php if (!empty($article['is_sponsored'])): ?>
      <span class="rounded-full bg-amber-100 px-2.5 py-1 text-[11px] tracking-widest text-amber-800" title="Contenu publié en partenariat avec un annonceur">Sponsorisé</span>
    <?php endif; ?>
  </p>
  <h1 class="mt-3 text-4xl leading-tight font-extrabold tracking-tight text-balance md:text-6xl"><?= htmlspecialchars($article['title']) ?></h1>
  <?php if (!empty($article['excerpt'])): ?>
    <p class="mt-5 font-serif text-xl leading-relaxed text-slate-600"><?= htmlspecialchars($article['excerpt']) ?></p>
  <?php endif; ?>

  <div class="mt-7 flex flex-wrap items-center justify-between gap-4 border-y border-slate-200 py-4">
  ...
  <?php if (!empty($article['hero_image'])): ?>
    <figure class="mt-8">
      <img class="aspect-video w-full cursor-zoom-in rounded-xl object-cover" data-lightbox src="<?= htmlspecialchars($article['hero_image']) ?>" alt="<?= htmlspecialchars($article['hero_alt'] ?? '') ?>" width="1200" height="675" fetchpriority="high">
      <?php if (!empty($article['hero_credit'])): ?>
        <figcaption class="mt-2 text-xs text-slate-400">© <?= htmlspecialchars($article['hero_credit']) ?></figcaption>
      <?php endif; ?>
    </figure>
  <?php endif; ?>
...
<?= $articleBody ?>
  ...
  <?php if (!empty($related)): ?>
    <section class="mt-14 border-t border-slate-200 pt-10">
      <h2 class="text-2xl font-extrabold tracking-tight">Articles associés</h2>
...
<?php require __DIR__ . '/partials/article-sidebar.php'; ?>
</div>
<script>
(() => {
